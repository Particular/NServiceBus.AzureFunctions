...
public sealed class MissingCompositionCallAnalyzer : DiagnosticAnalyzer
{
    ...
    {
        if (HasRegistrationType(compilation, compilation.Assembly))
        {
            return true;
        }
        ...
        foreach (var assembly in compilation.SourceModule.ReferencedAssemblySymbols)
        {
            ...
            if (HasRegistrationType(compilation, assembly))
            {
                return true;
            }
        }
        ...
        static bool HasRegistrationType(Compilation compilation, IAssemblySymbol assembly)
        {
            var details = CompilationAssemblyDetails.FromAssembly(assembly);
            return compilation.GetTypeByMetadataName($"NServiceBus.Generated.{details.ToGenerationClassName()}") is not null
                || compilation.GetTypeByMetadataName($"NServiceBus.Generated.{details.ToSendOnlyGenerationClassName()}") is not null;
        }
    }
