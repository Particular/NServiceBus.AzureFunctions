...
public class MessageProcessorTests
{
    ...
    [Test]
    public async Task Should_auto_generate_message_id_if_needed()
    {
        string? messageId = null;
        var result = await ProcessMessage(onMessage: (context, _) =>
        {
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.True(result.MessageActions.WasCompleted, "Message should be completed");
    ...
    public async Task Should_use_auto_generated_message_id_if_present()
    {
        ...
        {
            Assert.True(result.MessageActions.WasCompleted, "Message should be completed");
            Assert.AreEqual(messageId, autoGeneratedMessageId, "Auto generated message id should used as message id");
        }
    }

    [Test]
    public async Task Should_update_poison_dead_lettered_message_with_auto_generated_message_id()
    {
        Dictionary<string, object>? updatedProperties = [];
        var messageActions = new TestableMessageActions
        {
            DeadLetterMessage = (_, properties, _, _, _) =>
            {
        ...
        var result = await ProcessMessage(
            headerExtractor: _ => throw new Exception("simulated exception"),
            messageActions: messageActions);

        using (Assert.EnterMultipleScope())
        {
            Assert.True(result.MessageActions.WasDeadLettered, "Message should be dead lettered");
            Assert.IsTrue(updatedProperties.ContainsKey(Headers.MessageId), "MessageId property should be updated");
            var messageId = updatedProperties[Headers.MessageId].ToString();
            Assert.True(Guid.TryParse(messageId, out _), "MessageId should be a non null value");
        }
    }

    [Test]
    public async Task Should_update_application_dead_lettered_message_with_auto_generated_message_id()
    {
        Dictionary<string, object>? updatedProperties = [];
        string? autoGeneratedMessageId = null;
        ...
        var result = await ProcessMessage(
            ...
            onError: (context, _) =>
            {
                autoGeneratedMessageId = context.Message.NativeMessageId;
                context.TransportTransaction.Set(new DeadLetterRequest("reason", "description"));
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.True(result.MessageActions.WasDeadLettered, "Message should be dead lettered");
            Assert.IsTrue(updatedProperties.ContainsKey(Headers.MessageId), "MessageId property should be updated");
            Assert.AreEqual(updatedProperties[Headers.MessageId], autoGeneratedMessageId, "MessageId should be the auto generated value");
        }
    }
    ...
    public async Task Should_complete_messages_immediately_if_previous_processing_attempt_was_successful()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.IsTrue(firstProcessingResult.OnMessageWasCalled, "OnMessage should be called");
            Assert.IsTrue(firstProcessingResult.MessageActions.WasCompleted, "Message should be completed");
        }
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.IsFalse(secondProcessingResult.OnMessageWasCalled, "OnMessage should not be called");
            Assert.IsTrue(secondProcessingResult.MessageActions.WasCompleted, "Message should be completed");
        }
    }
    ...
    {
        IDictionary<string, object>? updatedProperties = new Dictionary<string, object>();
        string? autoGeneratedMessageId = null;

        var messageActions = new TestableMessageActions
        {
            AbandonMessage = (_, properties, _) =>
            {
                updatedProperties = properties;
                return Task.CompletedTask;
            }
